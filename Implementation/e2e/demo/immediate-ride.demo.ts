...
/**
 * **Scenario 1 del RASD — la corsa immediata**, guidata dai due client e fotografata (D76).
 *
 * Non è un test: nessun requisito dipende da lui, e `pnpm verify:e2e` non lo raccoglie
 * (`--project=chromium` in `tools/e2e/run.mjs`). È una dimostrazione riproducibile, che si esegue
 * con `pnpm demo:immediate` e lascia gli scatti in `e2e/screenshots/demo/scenario-1/`.
 *
 * Le asserzioni che ci sono servono a **fermarlo quando la storia non torna**: una demo che
 * prosegue dopo un passo fallito produce screenshot che mostrano il nulla, ed è peggio di un
 * errore. Ma non pretendono ciò che un test pretenderebbe — non c'è il conteggio delle richieste
 * di NFR2, che è affare del cancello di M8.
 *
 * **Gli scatti coprono i passaggi salienti, non ogni fase.** `searching` e `assigned` durano meno
 * del giro di campionamento: sono state colte in due esecuzioni su quattro, misurate. Fotografarle
 * si può solo scattando *prima* del ciclo, ed è ciò che il passo 4 fa per l'assegnazione. Promettere
 * uno screenshot per fase sarebbe promettere qualcosa che il sistema non garantisce.
 */
