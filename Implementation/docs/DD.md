...
| 1.10 | August 28, 2026 | **[v1.10]** One decision, taken when a review of the requirement coverage found a value that the system computed, persisted and never showed. RASD §2.3 lists «a clear overview of traffic levels» among what the operator needs, and `ModeController` writes the observed level on every reading (D20) — but no operation returned it, so it reached the database and never the screen. R12 was covered on the switching side and absent on the *visibility* side, which no test caught because no test looked at what the dashboard displays. `getMode()` now returns the mode **and** the last known traffic level, and the strategy panel shows it next to the strategy it explains. The dashboard **also re-reads it on an interval**: unlike the mode and the strategy, the level has no event per observation — Manual mode notifies nothing at all (R13) and the Medium alert fires on crossing the threshold, not on every reading (R12) — so listening alone would leave the badge frozen on the last event. See decision D75 in [Appendice A](#appendice-a--registro-delle-decisioni). |
| 1.11 | August 28, 2026 | **[v1.11]** One decision, and it is about being able to *show* the system rather than about how it works. Three of the four RASD scenarios could not be demonstrated on demand: advance booking waits fifteen minutes for its activation, rebalancing up to ten for its cycle, and the traffic sequence of scenario 3 could only be seen by turning up at 17:00 on a weekday, because the traffic level is deduced from Milan's local clock. The periodic cadences and the activation lead become configuration **with today's values as defaults**, and a second traffic adapter follows a table relative to process start, chosen by configuration. Production behaviour is unchanged. See decision D76 in [Appendice A](#appendice-a--registro-delle-decisioni). |
| 1.12 | August 29, 2026 | **[v1.12]** One decision, taken on a defect that no test reported and a demonstration made obvious: the operator's alert panel restarted empty at every reload, so a dashboard opened after six repositionings declared it had done nothing. The cause is that the four governance events have no passenger to be addressed to, while the `notification` table implements the RASD's `Notification`, which Figure 2.2 addresses to exactly one `Passenger` — its `recipient_id` is `NOT NULL` and its `type` must be one of five passenger-facing categories, so `record()` returned before writing. A separate `operator_alert` table now holds them, and a third port of `notifications` reads them back. See decision D77 in [Appendice A](#appendice-a--registro-delle-decisioni). |
| 1.13 | September 1, 2026 | **[v1.13]** One decision, taken on two defects that a demonstration exposed and no test could have reported, because both live at the seam between a token and the account behind it. A session whose account no longer exists — the ordinary state of any browser after `pnpm db:seed` — passed authentication and reached the database, where it became a foreign-key violation and a `500`; and a client whose reads all returned `401` could keep showing the fleet of its last successful read indefinitely. Authentication now confirms that the token's subject is still an account, which **supersedes the boundary asserted by the M1b gate** and is registered as such. See decision D78 in [Appendice A](#appendice-a--registro-delle-decisioni). |
| 1.14 | September 11, 2026 | **[v1.14]** One decision, taken because the model did not contain the phenomenon R12 exists for. Traffic switched the allocation strategy, but neither the fleet simulator nor any travel-time estimate read it: a vehicle took the same time in light or heavy traffic, and with a linear estimate the minimum-ETA strategy always picked the nearest vehicle — measured with the demonstration's request generator, **zero** assignments out of fifty-one differed from the nearest. Traffic now lives in the world: a per-zone level, confined to the `ExternalServicesGateway`, slows the simulated vehicles where they are and lengthens every estimate along the route, for any map provider. When enabled, each assignment also leaves a line in the
operator's log saying why that vehicle — strategy, estimated time, and what the nearest would have
taken. Everything new is off unless configured. See decision D79 in [Appendice A](#appendice-a--registro-delle-decisioni). |
...
- **NotificationManager**: `update(event)`; `registerSession(session)`, `removeSession(session)`,
  ...
  `update()` among its exported operations (decision D40). **[v1.12]** A **third port** publishes
  `recentAlerts(limit)`, the read path of the operator's alert history: the push channel reaches
  whoever is connected, and until v1.12 nothing else existed for operator alerts — an operator who
  reloaded the page found a dashboard that, after six repositionings, declared it had done nothing
  (decision D77).
...
- **ExternalServicesGateway**: `getETA()`, `getTraffic()`, `getDemandData()`, `commandRoute()`,
  ...
  **second port**, `FleetSimulationPort`, which advances the simulated world: it is the one thing
  ...
  day the vehicles are real (decision D64). **[v1.11]** The traffic provider now has **two**
  interchangeable adapters behind the same internal seam, chosen by configuration: the hourly one,
  which stays the default, and a scripted one that follows a table relative to process start so the
  hysteresis sequence of scenario 3 can be shown on demand. Neither appears in the module's
  `exports`, so no domain component can tell which is answering — which is the point: until v1.11
  the substitutability NFR8 promises had been asserted and never exercised (decision D76).
  **[v1.14]** Traffic is no longer only a label. When configured, the level has a **per-zone** detail
  — the centre follows the scripted table, the rest of the city stays `LOW` — and that detail slows
  the simulated vehicles where they are and lengthens every travel-time estimate by the share of the
  route that crosses congested zones, in the facade and therefore for any map provider. It never
  leaves the component: `getTraffic()` still returns **one** level, the centre's, which is the only
  one `ModeController` knows how to use. Unconfigured, nothing changes — the simulator runs its
  original advance loop and every estimate is multiplied by nothing (decision D79).
...
caller from the token and **confirms that the subject is still an account** [v1.13]; it consults no
session store, in memory or persistent, which is what makes the NFR3 proposition of Section 4.3 true
and testable. The distinction is the whole of decision D78: what that requirement forbids is state
held *in the memory of one process*, which would make replicas non-interchangeable, and a read of the
shared data tier is not that — a second instance runs the same query against the same database and
reaches the same verdict.

Two consequences worth stating, because they are what the design buys and what it costs:

- a token stays valid until it expires even if the account behind it **changes**: role and address
  remain the ones that were signed, and a profile edited a moment ago is not reflected until the
  token is renewed. The short lifetime is the mitigation. **[v1.13]** A token whose account has
  *disappeared* is a different case and is no longer accepted: there is nobody left to authenticate,
  and letting it through only deferred the refusal to a point that could no longer express it
  (decision D78);
...
class NotificationManager {
    ...
    + recentAlerts(limit: int): OperatorAlert[]
}
...
**Only one of the two audiences had a history [v1.12].** Both levels of the pattern deliver, and the
`notification` table is what survives a disconnection — its own comment says so: «having persisted it
is what lets a reconnecting client not lose what happened while it was away». But that table
implements the RASD's `Notification`, which Figure 2.2 addresses to exactly one `Passenger`, so its
`recipient_id` is `NOT NULL` and its `type` must be one of five passenger-facing categories. The four
governance events — an automatic strategy switch, a return to Auto, a traffic threshold reached, a
repositioning started — have neither, so `record()` returned before writing and **no operator alert
was ever persisted**. The push channel reached whoever was connected and nothing reached anyone else:
a dashboard reopened after six repositionings showed an empty panel. `operator_alert` is the sede
those events lacked, and `recentAlerts()` the way back to them (decision D77).

**A fifth decision reaches the operator, and deliberately not the history [v1.14].** When enabled,
every assignment leaves one line in the operational log: which strategy decided, with what estimated
time, and — when it did not pick the nearest candidate — how long the nearest would have taken. Like
the mode and rebalancing decisions it is not a `Subject` transition: `RideAllocator` calls `update()`
directly once the vehicle is assigned. It is **text only** — every structured field of the delivery
stays `null` except the vehicle — because the dashboard classifies any event carrying a strategy as a
switch and rewrites its strategy panel from it; as text it can at worst mislabel one line, as fields
it would move the screen. It is not persisted in `operator_alert`, which stays the four governance
events, and it is off unless configured, because the M5 gate asserts the exact list of deliveries the
dashboard receives (decision D79).
...
**[v1.5] What the realised cycle does differently.** The drawing above is the v1.0 one and is kept
for the shape of the flow; three messages differ from what M7 realises, and the same discipline this
...
- `getTraffic()` is **not** called either. The figure feeds it into `analyzeDemand()`, but decision
  D12 fixes the expected-demand metric as historical base × active event multipliers, with no
  traffic term: calling it would read a value that nothing consumes. Traffic drives the *allocation
  strategy* (R12), which is the `ModeController`'s business, not the rebalancer's;
- `commandRoute()` returns the route it commanded — its length and duration — and **not** an
  acceptance flag. `commandAccepted` in the drawing is the return of the call: no adapter can refuse
  a command today, and a field that nothing can set and nobody reads would be described behaviour
  rather than implemented behaviour.
...
shows **where the assigned robotaxi is**, as a marker joined by a straight segment to the point the
vehicle is heading for. The segment is a visual aid and not the route the vehicle will drive: the
passenger client is never told the planned path, only the current position, and the two must not be
confused.

**[v1.7]** The marker is shown for **every phase in which a vehicle of one's own exists** —
`assigned`, `arriving`, `arrived` and `in ride` — and the segment changes its far end when the
passenger boards: the pickup point before, the destination after. An earlier reading of this section
stopped the marker at boarding, on the argument that from then on the moving dot is the passenger
themselves. That is true and beside the point: the dot was not replaced by anything, so the map
emptied at the exact moment the ride began, which reads as a client that has lost its signal rather
than as a ride in progress. Pointing the segment at the pickup after boarding would be worse still —
it would mark a place already left behind. See decision D71.
...
**[v1.10]** The level reaches the panel two ways, and needs both. It arrives on the **push channel** with the events that carry it, and it is **re-read on an interval** alongside the mode. The second is not redundancy: the level is the only one of the three without an event per observation — in Manual mode `onTrafficLevel()` notifies nothing, which is what R13 asks for, and in Auto the Medium alert is emitted when the threshold is *reached* rather than on every reading, which is what R12 asks for. A client that only listened would therefore hold the last event for the whole session, and on a freshly seeded system would read «not detected» indefinitely. Emitting an event per reading was considered and rejected: it would fill the alerts panel with the same line for as long as the condition lasts (decision D75).
...
| R6 | Ride status notifications | NotificationManager + *Observer*; Robotaxi and Ride as `Subject`; RideRequestManager (`IRideLifecycleService`, `IFleetTelemetryService`); ExternalServicesGateway (the ETA carried by the arrival notice) **[v1.5]**; API Gateway (WebSocket push) |
...
D71–D73 di v1.7, D74 di v1.8, D75 di v1.10, D76 di v1.11, D77 di v1.12, D78 di v1.13 e D79 di v1.14.
Serve a due cose:
...
| D65 | L'annullamento **legge lo stato del veicolo prima di comandare qualcosa alla flotta**, e la telemetria comanda la rotta **dopo** la transizione | §2.4; decisione D59 | Due facce della stessa regola, e la prima nasce da un difetto trovato in revisione: revocare la rotta prima di sapere se la transizione sarà accettata fermava in strada il veicolo di un passeggero **già a bordo**, per poi rispondere che l'annullamento era rifiutato e che «nulla è stato scritto». Una revoca non è una scrittura che si possa disfare, quindi la si comanda solo sul cammino ammesso. La seconda faccia allinea `FleetTelemetry` all'ordine che la Figura 2.7 fissa per il riposizionamento — transizione, poi comando — perché la transizione è l'unico passo che può rifiutare senza lasciare traccia; il caso che sembrava richiedere l'ordine opposto, un veicolo in `ARRIVING` senza rotta, è chiuso dal ramo che la rotta la ricomanda |
| D66 | L'**ETA numerico** entra nel canale push con la transizione 4: `startPickupNavigation(robotaxiID, etaToPickupMinutes)`, e `NotificationDelivery` / `notificationPushSchema` prendono il campo | §2.2, §2.3.3; RASD R6; decisione D46 | Chiude la D46, che aveva rinviato il dato a «quando l'adapter OSRM e la telemetria sostituiscono il mock». Il numero lo conosce chi ha chiesto il percorso al fornitore — `FleetTelemetry` —, ma a notificarlo dev'essere il **veicolo**, perché «il tuo robotaxi sta arrivando» e «fra sette minuti» sono la stessa notizia e il passeggero deve riceverne una sola: un secondo evento avrebbe prodotto due push per un fatto solo, che è la cosa che la D45 e la nota su `AVAILABLE` in `notification-copy.ts` evitano altrove. Perciò il tempo viaggia **come argomento della transizione**, nella stessa forma in cui `assign()` riceve la corsa accettata: un valore che `fleet` non calcola, trasporta, e che serve a chi riceverà l'evento — senza che il modulo acquisti una dipendenza da `IExternalServices`, che la D61 gli ha deliberatamente tenuto lontana. Resta **annullabile**: se il fornitore non sa rispondere, la notifica torna a essere quella di M5, perché la disciplina della D46 vale anche quando il numero non esiste invece che quando non è credibile |
...
| D71 | Il marker del robotaxi **resta sulla mappa del passeggero anche a corsa cominciata**, e il segmento tratteggiato punta al ritiro prima della salita e alla destinazione dopo. La cadenza con cui le posizioni si producono e si rileggono è **una sola**, mezzo secondo, condivisa dal passo del simulatore, dalla lettura della telemetria e dai due client | §3.1, §3.2; RASD R3, R6, R7; NFR2; decisioni D61, D67, D69 | Due facce dello stesso difetto, entrambe visibili solo guardando lo schermo. La prima: la §3.1 in v1.6 fermava il marker all'imbarco, sul ragionamento che da lì in poi «il puntino è il passeggero» — vero, ma nessun puntino prendeva il suo posto, quindi la mappa si svuotava nell'istante in cui la corsa comincia, cioè quando l'attenzione di chi guarda è massima. La seconda: le posizioni si producevano ogni dieci secondi e i client le rileggevano ogni cinque, così un veicolo *saltava* invece di scorrere, e le due cadenze potevano divergere senza che nulla lo segnalasse. Tenerle in un'unica costante di `packages/shared` è ciò che le lega: leggere più di rado di quanto il mondo si muova nasconde un pezzo di strada, più spesso ripete la stessa risposta. **Il mondo simulato non è diventato più veloce** — resta sei volte il tempo reale — è il passo a essersi accorciato di venti volte, e questo non tocca né quale veicolo viene scelto né in quale ordine avvengono le transizioni. Una conseguenza non è un dettaglio di presentazione ed è stata chiusa dove andava chiusa: la D63 assumeva che il passeggero salisse «al giro successivo all'arrivo», e con il giro a mezzo secondo lo stato `arrived` sarebbe stato visibile mezzo secondo invece di dieci. La D63 esprime ora quella sosta in **secondi**, che è il posto giusto — la cadenza di osservazione e la durata di una salita non sono la stessa grandezza, e legarle era ciò che rendeva fragile l'assunzione |
...
| D75 | `ModeController.getMode()` restituisce il modo **e** l'ultimo livello di traffico noto, annullabile, invece del solo modo. Il pannello strategia della dashboard lo mostra come pastiglia colorata con etichetta, accompagnata da una riga che dice cosa il sistema ne sta facendo | §2.2.1, §3.2, Fig. 2.2; RASD §2.3, R12, R13; NFR9, NFR10; decisioni D4, D11, D20 | Un requisito coperto a metà, e la metà mancante era quella osservabile. Il RASD §2.3 elenca fra i bisogni dell'operatore «a clear overview of traffic levels», e `onTrafficLevel()` scrive il livello a ogni lettura (D20) — ma **nessuna operazione lo restituiva**: il valore arrivava al database e non allo schermo. R12 era verificato sul lato della commutazione, dove i cancelli lo esercitano a fondo, e assente sul lato della *visibilità*, che nessun test copriva perché nessun test guardava che cosa la dashboard mostra. Le due strade erano aggiungere un quinto lettore alla porta o allargare quello che c'era, e si è scelta la seconda per tre ragioni che convergono. La prima è di competenza: modo e livello sono due colonne dello **stesso** record `system_mode`, entrambe di `ModeController`, quindi l'obiezione che la §2.2.1 muove all'idea di una lettura unica per modo *e strategia* — «nessuno dei due componenti a cui il DD affida le due letture ha titolo per esporla», perché quelli sono due componenti diversi — qui non si applica. La seconda è di correttezza: una lettura sola tiene i due valori coerenti per costruzione, mentre una terza `SELECT` avrebbe aggiunto all'analisi di interleaving del `GET /mode` un caso nuovo — un livello letto prima di un cambio di modo e mostrato accanto a quello dopo. La terza è di economia: la porta resta alle **quattro** operazioni che la §2.2.1 le assegna. Il campo è **annullabile** perché «nessuna lettura ancora arrivata» è uno stato con un significato proprio e non un valore mancante: è la stessa distinzione su cui si regge la D11, che non rivaluta nulla quando il livello è nullo, e mostrare `LOW` al suo posto sarebbe un'affermazione che nessuno ha verificato. Il livello **non** giustifica la strategia attiva, e in modo Manual può contraddirla — là le osservazioni si registrano senza commutare niente (R13) — il che non è un difetto ma esattamente ciò che l'operatore deve vedere per decidere se il suo intervento ha ancora senso. **A differenza di modo e strategia, il livello non ha un evento per ogni osservazione**, ed è la ragione per cui la dashboard lo **rilegge a intervalli** oltre a riceverlo dagli eventi che lo portano. Le due metà mancanti sono entrambe volute: in modo Manual `onTrafficLevel()` non notifica affatto, perché R13 vuole che nulla di automatico accada; e in Auto l'alert di `MEDIUM` si emette **al passaggio di soglia** e non a ogni lettura, perché R12 parla di *raggiungere* una soglia e non di uno stato. Un client che si limitasse ad ascoltare resterebbe quindi fermo all'ultimo evento per tutta la sessione, e su un sistema appena seminato mostrerebbe «non rilevato» a tempo indeterminato: il `TrafficMonitor` scrive `LOW` al primo giro, `applySwitch` non commuta perché la strategia di default è già quella giusta, e nessun evento parte. L'alternativa considerata era **emettere un evento a ogni lettura**, ed è stata scartata perché riempirebbe il pannello alert della stessa riga per tutta la durata della condizione — che è esattamente la ragione per cui la D20 non lo fa già. La rilettura non contraddice NFR2: il push resta la via con cui i fatti discreti raggiungono un client connesso senza che lo chieda, e l'intervallo copre soltanto le finestre in cui, correttamente, non succede niente |
| D76 | Le **cadenze del lavoro periodico** e l'**anticipo di attivazione delle prenotazioni** diventano configurazione, con i valori attuali come default; e la sorgente del livello di traffico diventa **sostituibile**, con un secondo adapter `ScriptedTrafficGateway` che segue una tabella oraria relativa all'avvio del processo, scelto da `TRAFFIC_SOURCE` | §2.2, §2.4, §4.3; NFR8; RASD §1.2 (phenomena table), scenari 1–4; decisioni D8, D9, D12, D20, D54 | Le due metà sono la stessa idea — **rendere pilotabile il mondo esterno per poterlo dimostrare** — e per questo stanno in una decisione sola. M9 chiedeva i quattro scenari del RASD e li ha consegnati come suite di test: eseguibili, non mostrabili. Tre su quattro non erano dimostrabili a comando, e per ragioni della stessa natura: la prenotazione si attiva quando mancano quindici minuti all'orario e il controllo gira ogni minuto; il riposizionamento decide ogni dieci minuti; e il livello di traffico è una funzione dell'**ora locale di Milano**, quindi la sequenza dell'isteresi si vede solo presentandosi alle 17:00 di un feriale. Nessuno di questi numeri è sbagliato — le decisioni che li fissano li giustificano nel merito, e restano i default: un'installazione che ignora le nuove variabili si comporta esattamente come prima. Ciò che mancava era una manopola. **Per le cadenze** la scelta è stata leggere `process.env` nell'argomento di `@Cron` invece di registrare i job con `SchedulerRegistry`: la seconda strada, apparentemente più pulita perché la configurazione arriverebbe da `ConfigService`, richiede un `CronJob` dal pacchetto `cron`, che non è fra le dipendenze dichiarate né raggiungibile — sarebbe una libreria nuova per zero guadagno funzionale — e la variante con `addInterval` è esclusa dalla regola di determinismo, che esenta il solo `platform/`. L'obiezione alla via scelta è che un decoratore valuta il proprio argomento all'import, prima che `ConfigModule` legga il file d'ambiente, quindi funzionerebbe solo con variabili d'ambiente vere: è stata chiusa alla radice facendo caricare quel file a `pnpm dev` con `--env-file-if-exists`, così ogni valore scritto in `.env` **è** una variabile d'ambiente prima di qualunque import. È lo stesso principio con cui i due client leggono un solo file dalla radice: un file d'ambiente, nessun percorso privilegiato, nessuna regola implicita da ricordare. **Per il traffico** la forma è un secondo adapter e non una modifica al primo, che resta il default: `external` ospita già due adapter di percorso scelti dietro la porta, gli adapter sono provider interni che non compaiono negli `exports`, e nessun componente di dominio può sapere quale dei due stia rispondendo — quindi NFR8 ne esce **esercitato** invece che affermato, per la prima volta da quando è scritto. La tabella è relativa all'avvio e non a un orario assoluto, così lo stesso comando racconta la stessa storia a qualunque ora: è la condizione perché la dimostrazione sia riproducibile da terzi. L'alternativa scartata è **una rotta `POST /demo/traffic`**, che avrebbe risolto tutto dal browser: aggiungerebbe al contratto rotte che nessun requisito chiede, ma soprattutto **invertirebbe la classificazione dei fenomeni**. Il traffico è un fenomeno *del mondo*, osservato dalla macchina, e la phenomena table del RASD §1.2 lo dichiara così applicando Jackson; una rotta che lo *scrive* lo renderebbe un fenomeno controllato dalla macchina. Con l'adapter, invece, **è il mondo a essere diverso in dimostrazione, non la macchina** |
| D77 | Gli **alert dell'operatore** hanno una tabella propria, `operator_alert`, senza destinatario, e una **terza porta** di `notifications` (`recentAlerts`) per rileggerli. `notification` resta intatta | §2.2, §2.3.3, §3.2; RASD §2.2.3, Fig. 2.2, R7, R11, R12, R13; G8, G9; decisioni D40, D42 | Dal M6 a oggi **nessun alert dell'operatore è mai stato persistito**, e nessun test lo diceva: il pannello che il §3.2 descrive come «switch automatici e suggerimenti di riposizionamento» viveva della sola memoria della scheda, quindi ricaricare la pagina lo svuotava e chi apriva la dashboard dopo un riposizionamento trovava un sistema che dichiarava di non aver fatto niente. La causa è una **tensione che sta nel RASD**, non nel codice: la Figura 2.2 disegna `Notification "0..*" --> "1" Passenger : sent to`, e la tabella la realizza con `recipient_id uuid NOT NULL REFERENCES "user"` e `type` vincolato ai cinque `NotificationType`; ma fra quei cinque il RASD mette `REBALANCING_ALERT`, che è un avviso all'**operatore** e non ha nessun passeggero a cui andare. Il documento usa il concetto senza modellarlo, ed è la ragione per cui quel tipo non è mai stato emesso. Le alternative erano due. Rendere `recipient_id` annullabile con semantica di broadcast avrebbe reso **falsa a schema** la molteplicità che la figura dà per obbligatoria, e avrebbe caricato una colonna del doppio significato «nessuno» e «tutti». Scrivere una riga per operatore avrebbe conservato il vincolo, ma con due difetti: la scrittura si moltiplica per il numero di operatori a ogni evento, e soprattutto **chi viene creato dopo non vede il passato** — che per uno storico è il difetto peggiore possibile. La tabella a sé lascia vere entrambe le affermazioni: la `Notification` del RASD resta indirizzata a un passeggero, e l'alert dell'operatore diventa ciò che è, un fatto del sistema che chiunque apra la dashboard vede, anche chi si è registrato dopo. È anche l'unica forma in cui i campi che servono a rileggere — `strategy`, `mode`, `traffic_level`, `zone_id` — non sono colonne nulle per metà delle righe: il pannello classifica su quelli e non sul testo, quindi senza di loro una riga riletta si potrebbe mostrare ma non colorare. `zone_id` e `robotaxi_id` sono **copie e non chiavi esterne**, perché uno storico dice com'era il mondo allora: una `CASCADE` riscriverebbe il passato e una `SET NULL` lo impoverirebbe. Si persistono i **quattro eventi di governo** e non tutto il flusso dell'operatore, che è definito per esclusione e comprende ogni transizione di ogni veicolo: quelle si guardano sulla mappa mentre accadono, mentre ciò che serve rivedere è perché il sistema abbia deciso qualcosa. `REBALANCING_ALERT` resta quindi un tipo del RASD mai emesso, e questa decisione ne è la spiegazione invece che la conseguenza. **Disallineamento noto:** i sorgenti in `DD/diagrams/` non sono aggiornati — il riquadro `NotificationManager` di `class_notification.puml` elenca due operazioni invece di tre — e si riallineeranno alla rigenerazione dei PDF |
| D78 | L'autenticazione verifica, oltre a firma, scadenza e forma del payload, che il **soggetto del token sia ancora un account**. La lettura è una sola, incapsulata in `AccountLookup`, e la fanno entrambe le porte d'ingresso: i guard di D21 sul cammino HTTP e `verify()` di D41 sull'handshake della WebSocket. Guarda **solo l'esistenza**: ruolo e indirizzo restano quelli firmati. **D78 supera soltanto il confine asserito dal cancello di M1b** — il caso «e lo accetta senza consultare alcun registro, né in memoria né sul database», che cancellava la riga di `user` e pretendeva `200` — e lascia intatti sia il criterio di completamento di M1b sia la proposizione di NFR3 | §2.2.1, §4.3; RASD R1, R2, G1; NFR3; decisioni D21, D41, D68 | Due difetti, una sola domanda: che cosa vale un token il cui account non c'è più. Il primo lo produce **ogni `pnpm demo:*`**: `db:seed` svuota `user` e ricrea gli account con identificatori nuovi mentre il browser conserva il token di prima, che passava la verifica e arrivava fino al database, dove diventava una violazione di `ride_request_passenger_id_fkey` e un `500`. «Internal server error» davanti a un utente la cui unica colpa era non aver rifatto l'accesso, e senza alcun modo di capire che bastava quello. Tradurre la violazione di chiave esterna in un `401` sarebbe costato meno, ma agisce sul sintomo e **non raggiunge le letture**: `GET /fleet/status` non ha nessuna chiave esterna verso `user`, quindi la dashboard dell'operatore sarebbe rimasta esattamente nel secondo difetto. Il confine che il cancello asseriva non era una proprietà da conservare: era il difetto. Il suo commento lo chiamava «esattamente il confine fra le due cose» — autenticare non legge nulla, leggere o scrivere il profilo sì — e in quella formulazione si vede il costo, perché il rifiuto non spariva, si spostava in un punto dove non sapeva più esprimersi: `404` sul profilo, `500` sulla richiesta di corsa, `200` su ogni lettura. Con D78 il confine non si sposta, **sparisce**: tutte le rotte protette rispondono `401`, che è ciò che permette al client di avere una sola regola invece di tre. Quel che l'asserzione vecchia proteggeva — nessuna lettura, di nessun genere — era più severo del criterio di M1b (HARNESS.md §6: «un token emesso da un'istanza è accettato da una seconda che non ha visto il login») e più severo della §4.3, che vieta uno **store di sessione**; la stringa «né sul database» non compare in nessuno dei due documenti, e viveva solo dentro quel test e dentro un commento di `jwt.strategy.ts`, riscritto nello stesso commit. NFR3 resta verificato, e il caso riscritto lo verifica in una forma che quello vecchio non raggiungeva: le due istanze rifiutano identicamente un token il cui account è sparito **pur non essendosi mai parlate**, che è precisamente ciò che le rende intercambiabili. Il costo è una `SELECT` per richiesta autenticata, su chiave primaria, nello stesso database che la rotta interrogherà comunque un istante dopo. La lettura sta in una classe sola perché i due cammini rispondono in modo diverso — i guard sollevano, `verify()` restituisce `null` (D41) — e una socket che restasse aperta con un token che le rotte rifiutano continuerebbe a spedire eventi a chi non può più leggerli. **La metà client della stessa decisione** rende infine vera una promessa che D68 aveva già scritto — «entrambi i client tornano al login, comandi e letture comprese» — e che il codice manteneva solo a scheda in primo piano: `@tanstack/react-query` tiene in `pending`, non in `error`, una query che aspetta di ritentare, e sospende i tentativi a documento non visibile, quindi un `401` su una lettura periodica poteva non raggiungere mai chi lo osservava. Un `4xx` non è un guasto transitorio e non si ritenta: la regola sta in `packages/shared` accanto ad `ApiError`, così i due client non possono divergere |
| D79 | Il **traffico vive nel mondo**. Dentro l'`ExternalServicesGateway` il livello acquisisce un dettaglio **per zona** — le zone di `TRAFFIC_CENTRE_ZONES` seguono la tabella scriptata della D76, il resto della città resta `LOW` — e un fattore per livello (`TRAFFIC_TIME_FACTORS`, per esempio `MEDIUM:1.6,HIGH:4`) **rallenta il simulatore** secondo il punto in cui ogni veicolo si trova e **allunga ogni stima** dei tempi secondo la quota del tragitto che attraversa zone congestionate. `getTraffic()` continua a restituire **un livello solo**, quello del centro. Le due variabili sono assenti di default, e senza di esse niente cambia | §2.2; RASD R5, R8, R12; NFR8, NFR9; decisioni D54, D64, D76 | **Il modello non conteneva il fenomeno per cui R12 esiste**: sotto traffico il veicolo più vicino non è il più veloce. Il livello commutava la strategia, ma né il simulatore né le stime lo leggevano, e con la stima in linea d'aria il tempo è una funzione crescente della stessa distanza con cui ordina `NearestAvailableStrategy`: stesso ordinamento, stesso vincitore. Misurato con il generatore della dimostrazione, ETA minimo attivo per tutta la sequenza: **zero** assegnazioni diverse dal più vicino su cinquantuno. Il sistema commutava per una ragione che nel suo mondo non accadeva mai. **La regola del fattore è «lungo il tragitto»**: la linea da origine a destinazione si spezza in tratti di cinquanta metri e ognuno pesa per il fattore del proprio punto medio. È l'unica coerente con il simulatore, che spezza la strada con lo stesso passo e rallenta il veicolo secondo dove si trova: la stima prevede ciò che il simulatore farà. Le alternative sono state misurate sulla flotta del seed, 712 prelievi in centro, centro a `HIGH`: la **zona del prelievo** non cambia mai il vincitore — zero casi — perché moltiplica tutti i candidati per lo stesso numero; la **zona del veicolo** lo cambia in un caso su tre, ma promettendo tempi che il simulatore non mantiene, perché un'auto di periferia stimata veloce per tutto il tragitto rallenta appena entra in centro, e il registro dell'operatore mostrerebbe numeri falsi; la **media** fra le due sta nel mezzo con lo stesso difetto. Lungo il tragitto il vincitore cambia nel 13% dei prelievi a ×3 e nel 15% a ×4, sempre a favore di un'auto di periferia. **Il limite è fisica, non un difetto**: nel cuore del centro il più vicino vince comunque, perché qualunque auto venga da fuori deve attraversare il centro per arrivarci; il fenomeno esiste ai **bordi** del centro. Il fattore si applica **nel facade**, sopra entrambi i fornitori di mappe, perché il traffico è un fatto del mondo e non una proprietà di chi calcola i percorsi. Con OSRM la matrice dei tempi non porta la geometria, quindi la quota si calcola sul segmento retto: per un fornitore stradale è un'approssimazione, per la stima in linea d'aria è esattamente il percorso che il simulatore farà. Lo stesso fattore porta il tempo annunciato dal comando di rotta, cioè l'attesa promessa al passeggero. **Il dettaglio per zona non esce dal componente**, e non è una seconda operazione della porta: la strategia attiva è una per la città, e una soglia per zona non avrebbe niente da comandare. Resta nella giuntura interna `TrafficSource` della D76. Il livello restituito è quello **del centro** perché è lì che il più vicino smette di essere il più veloce, e quindi lì che una commutazione ha un senso. **I default non cambiano, per costruzione e non per aritmetica.** Senza `TRAFFIC_TIME_FACTORS` le stime sono quelle del fornitore, restituite senza campionare niente, e il simulatore non riceve alcuna funzione di rallentamento, quindi percorre il ciclo di avanzamento di prima: il cancello di M7, che conta i tick fino all'arrivo, non può spostarsi nemmeno per un arrotondamento in virgola mobile. Senza `TRAFFIC_CENTRE_ZONES` la tabella scriptata vale per tutta la città, come nella D76. Un fattore minore di uno, o una zona che non esiste, ferma l'avvio. **La prova non è la dimostrazione**, che gira su tempo reale: è un test d'integrazione con l'orologio finto, sulla catena intera e su Postgres vero. Con ETA minimo e il centro scorrevole nessuno dei 28 prelievi di una griglia regolare sul centro va a un veicolo diverso dal più vicino; con il centro a `HIGH` ci vanno in quattro, e per ciascuno la stima del veicolo scelto è minore di quella del più vicino. **Non cambia nessuna decisione del dominio**: `mode`, `allocation`, `rides`, `fleet` e `rebalancing` sono quelli di prima, e scelgono diversamente solo perché le stime che ricevono ora dicono il vero. **La decisione diventa leggibile** (`ALLOCATION_EXPLANATIONS=on`): dopo ogni assegnazione `RideAllocator` chiama `update()` con un evento `VEHICLE_ALLOCATED`, come fanno le decisioni di modo e di riposizionamento, e l'operatore legge nel registro una riga come «RT-22 assegnato con ETA minimo, 9,3 min — il più vicino, RT-06, ne avrebbe impiegati 10,5». Il più vicino si cerca fra i candidati **ottenibili** — lo scelto e quelli che l'allocatore aveva ancora davanti, esclusi quelli persi per concorrenza — e il suo tempo si chiede allo stesso stimatore, nello stesso istante. Fra tutti i candidati no, ed è stato misurato: in dimostrazione, con il più vicino appena perso per un tentativo fallito, una riga ha mostrato un distacco **negativo** — «il più vicino, RT-25, ne avrebbe impiegati 1,6» contro 1,9 — cioè un numero falso nel registro, confrontato con un veicolo che l'allocatore non poteva prendere. **`AllocationPort` non cambia**: cambiare il tipo di ritorno di `allocate()` avrebbe rotto il cancello di M3, che asserisce sul veicolo restituito. Il prezzo è una **finestra dichiarata**: la strategia si legge accanto ad `allocate()` e non dentro, quindi una commutazione che cadesse fra le due letture etichetterebbe male il testo di quella sola riga. È per questo che la spiegazione è **solo testo**: con `strategy` valorizzato l'evento sarebbe classificato come commutazione da `alertCategoryOf` e riscriverebbe il pannello strategia in `applyModeEvent` — e un'assegnazione a cavallo di una commutazione riporterebbe indietro il pannello proprio nell'istante che la dimostrazione vuole mostrare. Con i campi nulli, al peggio, una riga dice una cosa imprecisa. Non si persiste in `operator_alert`, che resta i quattro eventi di governo della D77. **È spenta di default, e la ragione è misurata**: accesa, ogni assegnazione produce una consegna in più alla dashboard, e il cancello di M5 — che asserisce la lista esatta di ciò che la dashboard riceve — fallisce alla riga che la confronta; spenta, passa otto su otto |
